 ...
 */
metadata {
	definition (name: "My Aeon Minimote", namespace: "okpowerman", author: "SmartThings") {
		capability "Actuator"
		capability "Button"
...
def buttonEvent(button, held) {
	...
	if (held) {
		createEvent(name: "button", value: "held", data: "${button}", descriptionText: "$device.displayName button $button was held", isStateChange: true)
	} else {
		createEvent(name: "button", value: "pushed", data: "${button}", descriptionText: "$device.displayName button $button was pushed", isStateChange: true)
	}
}
