...
metadata {
	...
	definition (name: "OKpowerman Thermostat", namespace: "OKpowerman", author: "SmartThings") {
	...
	tiles(scale: 2) {
		...
		standardTile("fanMode", "device.thermostatFanMode", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
			state "fan: Auto", label:'${name}', action:"thermostat.fanOn", backgroundColor:"#ffffff"
			state "fan: On", label:'${name}', action:"thermostat.fanCirculate", backgroundColor:"#ffffff"
			state "fan: Circulate", label:'${name}', action:"thermostat.fanAuto", backgroundColor:"#ffffff"
		}
        ...
        standardTile("refresh", "switch.refresh", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
			state "default", label:'Refresh', action:"refresh", backgroundColor:"#ffffff"		
		}

		main("thermostatMulti")
		details([
        	"thermostatMulti",
			"fanMode", "heatDown", "heatUp",
            "operatingState",
			"coolDown", "coolUp",
            "refresh"
		])
...
//			"coolingSetpoint", "coolDown", "coolUp","refresh"
//		])        
...
def setHumidity(humid) {
	log.debug "Setting humidity to ${humid}%"
	sendEvent(name: "humidity", value: humid, unit: "%")
...
def refresh() {
	if(device.currentValue("switch")=="refresh") {
    	refreshDone()
    }
	sendEvent([name:"switch", value: "refresh"])
    log.debug "Sent Refresh switch refresh"
    runIn(60, refreshDone)
    return
}
def refreshDone() {
...
def tempUp() {
	def cm = device.currentValue("thermostatMode")
    log.debug "cm is ${cm}"
    if (cm in ["heat","emergency heat"]) {
    	log.debug "Heat Mode"
        heatUp()
    }
    else if  (cm in ["cool"]) {
    	...
    	log.error "Mode is ${cm} USE absolute buttons"
    }
}
...
def setTemperature(value) {
	def ts = device.currentValue("temperature")
...
def heatUp() {
    ...
    log.debug "Sending heatingSetpoint ${value}"
...
def coolUp() {
    ...
    log.debug "Sending coolingSetpoint ${value}"
