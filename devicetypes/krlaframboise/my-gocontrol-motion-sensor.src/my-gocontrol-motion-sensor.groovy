 ...
 */
metadata {
	definition (name:"My GoControl Motion Sensor", namespace:"krlaframboise", author: "Kevin LaFramboise") {
		capability "Sensor"
		capability "Battery"
...
def configure() {	
	...
	cmds += [
        ...
        //wakeUpIntervalCapabilitiesGetCmd(),
...
def refresh() {	
    ...
    logTrace "refresh: ${canSendConfiguration()}"
}
...
private retriggerWaitTimeSetCmd() {
	...
	return secureCmd(zwave.configurationV1.configurationSet(scaledConfigurationValue: retriggerWaitTimeSetting, parameterNumber: 1, size: 1))	
}
private wakeUpIntervalGetCmd() {
	
	logTrace "wakeUpIntervalGet"
	
	return secureCmd(zwave.wakeUpV2.wakeUpIntervalGet())
}
private wakeUpIntervalCapabilitiesGetCmd() {
	logTrace "wakeUpIntervalCapabilitiesGet"
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv2.WakeUpIntervalCapabilitiesReport cmd) {
	logDebug "WakupIntervalCapabilitiesReport: $cmd"
	return []
}
def zwaveEvent(physicalgraph.zwave.commands.wakeupv2.WakeUpIntervalReport cmd) {
	logDebug "WakupIntervalReport: $cmd"
	return []
}
...
private temperatureGetCmd() {
	return secureCmd(zwave.sensorMultilevelV5.sensorMultilevelGet(sensorType:0x01, scale:0x00)) // get temp 
...
private configBulkGetCmd() {
	logTrace "Requesting bulk configuration report"
	return secureCmd(zwave.configurationV2.configurationBulkGet())
}
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv1.WakeUpNotification cmd)
{
	...
	if (canSendConfiguration()) {
    	logTrace "wakupNotifcation pendingConfig is true"
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv2.ConfigurationBulkReport cmd) {
	logDebug "Configuration Bulk Report: $cmd"
	return []
}
...
private canSendConfiguration() {
	logTrace "Check canSend Config : ${state.isConfigured} , ${state.pendingRefresh} , ${state.pendingChanges} "
...
{
	logTrace "WakeUpNotificationV2: $cmd"
	def result = []
	...
	if (canSendConfiguration()) {
		...
		logTrace "Skipping battery check because it was already checked within the last $reportBatteryEvery hours."
	}
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd) {
	...
	switch (cmd.sensorType) {
		case 1:
            ...
            if (cmdScale == "C" && getTemperatureScale() == "F") {
            	myVal = cmd.scaledSensorValue*9/5 + 32 + temperatureOffset
				map.value = myVal
            } else {
            	myVal = cmd.scaledSensorValue + temperatureOffset
                map.value = myVal
            }         
