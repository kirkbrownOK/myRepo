...
preferences {
	input "minutesBeforeEmergency", "number", title:"Number of minutes before device is considered in emergency", defaultValue:"120", required:false, displayDuringSetup:true
}
metadata {
	definition (name: "My CentraLite Thermostat", namespace: "smartthings", author: "SmartThings") {
		capability "Actuator"
	...
	tiles {
		...
		main "temperature"
		details(["temperature", "mode", "fanMode", "heatSliderControl", "heatingSetpoint", "coolSliderControl", "coolingSetpoint", "refresh", "configure","state"])
	}
}
...
def parse(String description) {
	...
	attrData.each {
		...
		if (map) {
            ...
            brokeMap.name = "malfunction"
            brokeMap.value = "normal"
            result << createEvent(brokeMap)
		}
...
def poll() {
    ...
    def latest = device.currentState("temperature")?.date?.time
    log.debug "Last temp update: ${latest} and now is ${now()}"
    if( now() - latest > ( 2 * state.minutes * 60 * 1000) ){
    	def brokeMap = [:]
        brokeMap.name = "malfunction"
        brokeMap.value = "emergency"
        sendEvent(brokeMap)
    }
}
...
def updated() {
	if(minutesBeforeEmergency) {
    	state.minutes = minutesBeforeEmergency
    } else {
    	state.minutes = 36000
    }
}
