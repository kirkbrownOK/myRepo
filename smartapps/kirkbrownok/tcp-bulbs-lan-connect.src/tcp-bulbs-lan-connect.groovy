/**
 *  TCP Bulbs LAN (Connect)
 *
 *  Copyright 2014 Todd Wackford
 *	Updated August 2016 by Kirk Brown
 *
...
def setupBulbs() {
	...
	selectedBulbs.each { did ->
		...
		if(!d) {
			...
			d = addChildDevice("kirkbrownOK", deviceFile, did, null, [name: "${newBulb?.name}", label: "${newBulb?.name}", completedSetup: true])
